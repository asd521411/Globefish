...
#endif
#define kDefaultUndoLevelMax 20 
#define kAutoScrollMinimumDuration 0.1 
#define kLongPressMinimumDuration 0.5 
#define kLongPressAllowableMovement 10.0 
#define kMagnifierRangedTrackFix -6.0 
#define kMagnifierRangedPopoverOffset 4.0 
#define kMagnifierRangedCaptureOffset -6.0 
...
@interface YYTextView () <UIScrollViewDelegate, UIAlertViewDelegate, YYTextDebugTarget, YYTextKeyboardObserver> {
    ...
    NSInteger _autoScrollAcceleration; 
...
- (void)_updateLayout {
    ...
    if (_innerText.length == 0) {
        [text yy_setAttributes:_typingAttributesHolder.yy_attributes]; 
    }
    ...
    if (_selectionView.caretVisible) {
        ...
        if (mgr.keyboardVisible) {
            ...
            if (!CGRectIsNull(kbInter) && kbInter.size.height > 1 && kbInter.size.width > 1) {
                if (CGRectGetMinY(kbInter) > CGRectGetMinY(rect)) { 
                    rect.size.height -= kbInter.size.height;
                } else if (CGRectGetMaxY(kbInter) < CGRectGetMaxY(rect)) { 
...
- (void)_hideMenu {
    ...
    if (_containerView.isFirstResponder) {
        ...
        [_containerView resignFirstResponder]; 
...
- (void)_scrollRangeToVisible:(YYTextRange *)range {
    ...
    if (mgr.keyboardVisible && self.window && self.superview && self.isFirstResponder && !_verticalForm) {
        ...
        if (!CGRectIsNull(inter) && inter.size.height > 1 && inter.size.width > extend) { 
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (_state.trackingTouch) {
        ...
        if (_highlight) {
            ...
            if ([_selectionView isGrabberContainsPoint:point]) { 
                self.panGestureRecognizer.enabled = NO; 
                [self _hideMenu];
                ...
                if (_selectedTextRange.asRange.length == 0 && self.isFirstResponder) {
                    if ([_selectionView isCaretContainsPoint:point]) { 
                        _state.trackingCaret = YES;
                        self.panGestureRecognizer.enabled = NO; 
                    }
                }
