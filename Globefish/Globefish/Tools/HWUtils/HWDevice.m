...
+ (NSDictionary *)clientInfo
{
    ...
    NSString *appVersion = [AppUpdater localVersion];
...
+ (EDeviceType)getDeviceType
{
    ...
    if (height <= 480) {
        return EDeviceType_4;
    }
    else if (height > 480 && height <= 568)
    {
        return EDeviceType_5;
    }
    else if (height > 568 && height <= 667)
    {
        return EDeviceType_6;
    }
    else if (height > 667 && height <= 736)
    {
        return EDeviceType_6p;
    }else
    {
        return EDeviceType_X;
    }
}
+ (BOOL)isUnderIPhone5
{
...
+ (BOOL)isIOS8
{
    return [HWDevice systemVersion] >= 8.0;
}
@end
